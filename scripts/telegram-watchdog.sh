#!/usr/bin/env bash
# telegram-watchdog.sh — Keep Telegram tmux sessions alive
#
# Runs as a background daemon. Respawns dead sessions on interval.
# Unlike the Discord watchdog, there is no discover cycle — Telegram Bot API
# has no channel listing endpoint, so sessions are spawned manually.
...
cmd_run() {
  _log "Watchdog started (interval=${INTERVAL}s, pid=$$)"
